    ...
    methods
        ...
        function obj = predict(obj,motionmodel,P_S,birthmodel)
            ...
            for i=1:length(birthmodel)
                ...
                dummy_states.P = motionmodel.F(birthmodel(i).x) * birthmodel(i).P * motionmodel.F(birthmodel(i).x)' + motionmodel.Q;
            ...
            for i=1:length(obj.paras.w)
                ...
                dummy_states.P = motionmodel.F(obj.paras.states(i).x) * obj.paras.states(i).P * motionmodel.F(obj.paras.states(i).x)' + motionmodel.Q;
                obj.paras.states = cat(1, obj.paras.states, dummy_states);
            end
            % for i=1:length(birthmodel)
            %     obj.paras.w = cat(1, obj.paras.w, birthmodel(i).w);
        ...
        function obj = update(obj,z,measmodel,intensity_c,P_D,gating)
            ...
            for i=1:hypotheses_num
                ...
                dummy_S = measmodel.R + measmodel.H(dummy_states.x) * dummy_states.P * measmodel.H(dummy_states.x)';
                S = cat(3, S, (dummy_S + dummy_S') / 2);
                K = cat(3, K, dummy_states.P * measmodel.H(dummy_states.x)' / S(:, :, i));
                P = cat(3, P, (eye(state_dim) - K(:, :, i) * measmodel.H(dummy_states.x)) * dummy_states.P);
